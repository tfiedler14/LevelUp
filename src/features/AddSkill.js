...
export const AddSkill = ({ skills, setLocation, location, handleSubmit }) => {
  ...
  const handleAttrValue = attr => {
    placeHolder.attr = attr;
    console.log(placeHolder.attr);
  };
  //currently undefined
  const handleAttrDef = def => {
    placeHolder.def = def;
    console.log(placeHolder.def);
  };
  const handleSkillSave = location => {
    console.log('saving new skill');
    setLocation('profile');
  };

  const attrValue = placeHolder.value;
      ...
      <View>
        ...
        <View style={styles.cardPadding}>
          ...
          <RNPickerSelect
            ...
            onValueChange={value => this.handleAttrValue(value)}
            items={[
          ...
          <Field name="addSkill" id="addSkill" component={WrappedTextInput} />
        </View>
        <View>
          <Button
            color="#fff"
